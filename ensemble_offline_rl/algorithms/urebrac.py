    ...
    def _train_step(runner_state, _):
        ...
        def _update_critics(runner_state, _):
            ...
            def _compute_target(rng, transition):
                ...
                target_q_std = target_q.std(axis=-1, keepdims=True) # [B, 1]
                next_q_value = jnp.expand_dims(1.0 - transition.done, -1) * (target_q - args.beta_id * target_q_std - args.critic_bc_coef * bc_loss)
                return jnp.expand_dims(transition.reward, -1) + args.gamma * next_q_value, bc_loss
            ...
            def _q_loss_fn(params):
                ...
                q_loss = jnp.square(q_pred - targets_expanded).sum(-1)
                return q_loss.mean()
...
def train(args):
    ...
    q_net = EnsembleQNetwork(obs_mean, obs_std, args.critic_ln, args.norm_obs, args.num_critics)
